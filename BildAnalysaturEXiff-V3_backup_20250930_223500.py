...
class OCRReviewApp(tk.Tk):
    ...
    def create_menu_bar(self):
        ...
        file_menu.add_command(label="Ordner öffnen...", command=self.open_folder, accelerator="Ctrl+O")
        file_menu.add_separator()
        file_menu.add_command(label="Excel Grunddaten laden...", command=self.show_excel_dialog)
        file_menu.add_separator()
        ...
        edit_menu.add_command(label="Aktualisieren", command=self.refresh_images, accelerator="F5")
        ...
        analysis_menu.add_command(label="OCR-Analyse starten", command=self.auto_analyze, accelerator="Ctrl+A")
        ...
        view_menu.add_command(label="Zoom vergrößern", command=self.zoom_in, accelerator="Ctrl++")
        view_menu.add_command(label="Zoom verkleinern", command=self.zoom_out, accelerator="Ctrl+-")
        view_menu.add_command(label="Zoom zurücksetzen", command=self.zoom_reset, accelerator="Ctrl+0")
        view_menu.add_separator()
        view_menu.add_command(label="Vollbild", command=self.toggle_fullscreen, accelerator="F11")
        
        # Extras-Menü
        tools_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Extras", menu=tools_menu)
        tools_menu.add_command(label="Einstellungen...", command=self.open_config_editor, accelerator="Ctrl+,")
        tools_menu.add_separator()
        tools_menu.add_command(label="OCR Log anzeigen", command=self.show_ocr_log)
        tools_menu.add_command(label="Detailliertes Log anzeigen", command=self.show_detailed_log)
        tools_menu.add_separator()
...
Ctrl+,    - Einstellungen öffnen"""
    ...
    def create_widgets(self):
        ...
        # Wichtigste Buttons in der Toolbar
        ttk.Button(left_toolbar, text="📁 Ordner öffnen", command=self.open_folder).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(left_toolbar, text="📊 Excel laden", command=self.show_excel_dialog).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(left_toolbar, text="🔍 Analysieren", command=self.auto_analyze).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(left_toolbar, text="🔄 Aktualisieren", command=self.refresh_images).pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Button(right_toolbar, text="📋 OCR Log", command=self.show_ocr_log).pack(side=tk.RIGHT, padx=(5, 0))
        ttk.Button(right_toolbar, text="📝 Log", command=self.show_detailed_log).pack(side=tk.RIGHT, padx=(5, 0))
        ttk.Button(right_toolbar, text="⚙️ Einstellungen", command=self.open_config_editor).pack(side=tk.RIGHT, padx=(5, 0))
        
        # Ordner Label unterhalb der Toolbar
        self.label_folder = ttk.Label(toolbar_frame, text="Kein Ordner ausgewählt")
        self.label_folder.pack(side=tk.TOP, padx=(10, 0), pady=(5, 0))
        ...
        content_frame.grid_rowconfigure(0, weight=1)  # Hauptinhalt expandiert vertikal
        ...
        left_column.grid_columnconfigure(0, weight=1)  # Spalte expandiert horizontal
        left_column.grid_rowconfigure(1, weight=1)  # Bild-Bereich expandiert vertikal
        ...
        self.canvas.create_text(400, 250, text="Bitte wählen Sie einen Ordner mit Bildern aus", 
        ...
        # Canvas-Größe setzen
        self.canvas.configure(width=800, height=500)
        ...
        nav_frame.grid(row=2, column=0, sticky="ew", pady=(0, 5))
        
        # Navigation mittig anordnen
        nav_center_frame = ttk.Frame(nav_frame)
        nav_center_frame.pack(expand=True)
        
        # Vorher Button (größer, blaue Farbe)
        prev_button = tk.Button(nav_center_frame, text="◀ Vorher", command=self.prev_image, 
                                bg="#2196F3", fg="white", font=("TkDefaultFont", 12, "bold"), width=15, height=2)
        prev_button.pack(side=tk.LEFT, padx=(0, 10))
        
        # Zoom Button (größer, orange Farbe)
        zoom_button = tk.Button(nav_center_frame, text="Zoom & Markieren", command=self.open_zoom_window, 
                               bg="#FF9800", fg="white", font=("TkDefaultFont", 12, "bold"), width=15, height=2)
        zoom_button.pack(side=tk.LEFT, padx=(0, 10))
        
        # Korrekt Dropdown
        ttk.Label(nav_center_frame, text="Korrekt:").pack(side=tk.LEFT, padx=(0, 2))
        self.correct_var = tk.StringVar()
        self.correct_combo = ttk.Combobox(nav_center_frame, values=self.valid_kurzel, textvariable=self.correct_var, width=12)
        self.correct_combo.pack(side=tk.LEFT, padx=(0, 10))
        self.correct_combo.bind("<<ComboboxSelected>>", self.on_correct_changed)
        
        # Nächste Button (größer, lila Farbe)
        next_button = tk.Button(nav_center_frame, text="Nächste ▶", command=self.next_image, 
                                bg="#9C27B0", fg="white", font=("TkDefaultFont", 12, "bold"), width=15, height=2)
        next_button.pack(side=tk.LEFT, padx=(0, 10))
        
        # Grüner OK-Knopf (Inordnung)
        ok_button = tk.Button(nav_center_frame, text="✓ In Ordnung", command=self.mark_as_ok_and_next, 
                              bg="#4CAF50", fg="white", font=("TkDefaultFont", 12, "bold"), width=15, height=2)
        ok_button.pack(side=tk.LEFT)
        
        # Kleines Icon für "Bild nicht verwenden"
        skip_button = tk.Button(nav_center_frame, text="🚫", command=self.mark_as_skip_and_next, 
                               bg="#f44336", fg="white", font=("TkDefaultFont", 12, "bold"), width=8, height=2)
        ...
        desc_frame.grid(row=3, column=0, sticky="ew", pady=(0, 5))
        ...
        # Konfiguriere Spalten- und Zeilengewichte für vertikales Resizing
        center_column.grid_columnconfigure(0, weight=1)  # Spalte expandiert horizontal
        center_column.grid_rowconfigure(0, weight=1)  # Erste Zeile expandiert vertikal
        
        # Container-Frame für vertikale Zentrierung
        center_container = ttk.Frame(center_column)
        ...
        # Bild verwenden
        use_frame = ttk.LabelFrame(center_container, text="Bild verwenden", padding=5)
        use_frame.grid(row=1, column=0, sticky="ew", pady=(0, 5))
        ...
        # Schadenskategorien
        damage_frame = ttk.LabelFrame(center_container, text="Schadenskategorien", padding=5)
        damage_frame.grid(row=2, column=0, sticky="ew", pady=(0, 5))
        self.damage_vars = {}
        ...
        # Bildart-Kategorien
        image_type_frame = ttk.LabelFrame(center_container, text="Bildart-Kategorien", padding=5)
        image_type_frame.grid(row=3, column=0, sticky="ew", pady=(0, 5))
        ...
        # Schadensbewertungs-Bewertung
        quality_frame = ttk.LabelFrame(center_container, text="Schadensbewertung", padding=5)
        quality_frame.grid(row=4, column=0, sticky="ew")
        ...
        # Konfiguriere Spalten- und Zeilengewichte für vertikales Resizing
        right_column.grid_columnconfigure(0, weight=1)  # Spalte expandiert horizontal
        right_column.grid_rowconfigure(0, weight=0)  # Anzahl-Bereich nimmt nur benötigten Platz
        right_column.grid_rowconfigure(1, weight=1)  # Treeview-Bereich expandiert vertikal
        ...
        categories_frame = ttk.LabelFrame(right_column, text="Kategorien-Fortschritt", padding=5)
        ...
        # Konfiguriere Tags für grüne Hintergrundfarbe (ganze Zeile)
    ...
    def is_image_evaluated(self, exif_data):
        ...
        if any(variant in damage_categories for variant in no_defects_variants):
            print(f"DEBUG: Bild als bewertet erkannt wegen 'keine Defekte': {damage_categories}")
            return True
        ...
        if is_evaluated:
            print(f"DEBUG: Bild als bewertet erkannt wegen Schäden+Bildarten: Schäden={damage_categories}, Bildarten={image_types}")
    ...
    def center_image(self):
        ...
        try:
            if hasattr(self, 'canvas') and self.canvas and self.photo:
                ...
                # Lösche alle Items und zeige Bild zentriert
