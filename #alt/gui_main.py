...
from utils_logging import get_logger, write_detailed_log, write_log_entry
_log = get_logger('app', {"module": "gui_main"})
...
Ctrl+,    - Einstellungen öffnen"""
    ...
    def update_categories_treeview(self):
        ...
        try:
            ...
            _log.error(f"Fehler beim Aktualisieren des Kategorien-Treeviews: {e}", extra={"event": "ui_error"})
    ...
    def calculate_kurzel_progress(self, kurzel_code):
        ...
        try:
            ...
            _log.error(f"Fehler beim Berechnen des Kürzel-Fortschritts: {e}", extra={"event": "ui_error"})
            return "0/0"
    ...
    def on_treeview_double_click(self, event):
        ...
        try:
            ...
            if parent:  # Es ist ein Kürzel (Unterelement)
                _log.info(f"Doppelklick auf Kürzel: {item_text}", extra={"event": "ui_double_click_kurzel"})
                self.navigate_to_kurzel(item_text)
            else:  # Es ist eine Kategorie (Hauptelement)
                _log.info(f"Doppelklick auf Kategorie: {item_text}", extra={"event": "ui_double_click_category"})
                self.navigate_to_category(item_text)
                
        except Exception as e:
            _log.error(f"Fehler beim Doppelklick-Handler: {e}", extra={"event": "ui_error"})
    ...
    def navigate_to_kurzel(self, kurzel_code):
        ...
        try:
            ...
            for i, filename in enumerate(self.files):
                if filename in self._evaluation_cache:
                    ...
                    if cache_entry.get('tag_ocr') == kurzel_code:
                        ...
                        self.index = i
                        self.show_image()
                        _log.info(f"Zu Bild gesprungen: {filename} (Index: {i})", extra={"event": "ui_jump_to_image"})
                        return
            ...
            _log.error(f"Fehler beim Navigieren zu Kürzel {kurzel_code}: {e}", extra={"event": "ui_error"})
    ...
    def navigate_to_category(self, category_name):
        ...
        try:
            ...
            for i, filename in enumerate(self.files):
                if filename in self._evaluation_cache:
                    ...
                    if tag_ocr in category_kurzel:
                        ...
                        self.index = i
                        self.show_image()
                        _log.info(f"Zu Bild der Kategorie gesprungen: {filename} (Index: {i}, Kürzel: {tag_ocr})", extra={"event": "ui_jump_to_category_image"})
                        return
            ...
            _log.error(f"Fehler beim Navigieren zu Kategorie {category_name}: {e}", extra={"event": "ui_error"})
    ...
    def show_detailed_log(self):
        ...
        try:
            # Neuer JSON-Logger: zeige Hinweis
            log_content = "Detailliertes Log wird jetzt als JSON in detailed_log.txt geführt. Ein eingebauter Viewer folgt."
        except Exception as e:
    ...
    def clear_detailed_log(self, log_window):
        ...
        if messagebox.askyesno("Bestätigung", "Möchten Sie das detaillierte Log wirklich löschen?"):
            try:
                # In neuer Struktur ggf. detailed_log.txt leeren – hier nur Hinweis
    ...
    def show_ocr_log(self):
        ...
        # Hinweis: OCR-Log ist JSON in ocr_log.txt
        ocr_path = os.path.join(os.path.dirname(__file__), 'ocr_log.txt')
    ...
    def load_current_evaluation(self):
        ...
        if auto_selected_types and not image_types:  # Nur wenn noch keine Bildarten gesetzt sind
            ...
            _log.info(f"Automatische Bildart-Vorauswahl: {image_types}", extra={"event": "auto_image_type"})
        ...
        try:
            ...
            for assignment in assignments:
                if assignment in IMAGE_TYPES:
                    ...
                    _log.warning(f"Unbekannte Bildart-Zuordnung: {assignment}", extra={"event": "auto_image_type_unknown"})
            
            return selected_types
            
        except Exception as e:
            _log.error(f"Fehler bei automatischer Bildart-Vorauswahl: {e}", extra={"event": "ui_error"})
            return []
