...
def find_text_box_easyocr(img_bgr, crop_coords: dict = None, valid_kurzel: list = None, detect_params: DetectParams = None):
    ...
    try:
        ...
        if not hasattr(find_text_box_easyocr, 'reader'):
            ...
            log_ocr.info("OCR Reader für Boxsuche init", extra={"lang": ["en"], "gpu": False, "event": "ocr_reader_box"})
        H, W = img_bgr.shape[:2]
        ...
        log_ocr.info("Boxsuche ROI gelesen", extra={"count": len(result), "event": "ocr_readtext_roi"})
        if not result:
            return None
        ...
        if valid_kurzel:
            try:
                ...
                log_ocr.info("Filter gültige Kürzel", extra={"count": len(valid_norm), "event": "ocr_filter_codes"})
            except Exception:
        ...
        for idx, detection in enumerate(result):
            ...
            if area_frac < dp.min_area_frac or area_frac > dp.max_area_frac:
                log_ocr.debug("Box abgelehnt (area)", extra={"area_frac": area_frac, "event": "ocr_box_reject_area"})
                continue

            # Filter: Seitenverhältnis (Breite/Höhe)
            aspect = (w / max(1.0, h)) if h > 0 else 0.0
            if aspect < dp.min_aspect or aspect > dp.max_aspect:
                log_ocr.debug("Box abgelehnt (aspect)", extra={"aspect": aspect, "event": "ocr_box_reject_aspect"})
                continue
            ...
            if valid_norm is not None:
                ...
                if not any(k in text_clean for k in valid_norm):
                    log_ocr.debug("Text passt zu keinem Code", extra={"text": text_clean, "event": "ocr_text_reject"})
                    continue
            
            # Score basierend auf Confidence
            score = float(confidence) * 100.0
            log_ocr.debug("Box akzeptiert", extra={"score": score, "event": "ocr_box_accept"})
            
            if score > best_score:
                best_score = score
                best_box = (int(x1_abs), int(y1_abs), int(w), int(h))
                log_ocr.debug("Neuer bester Treffer", extra={"box": best_box, "event": "ocr_box_best"})
        ...
        log_ocr.exception("EasyOCR Box-Detection Fehler", extra={"event": "ocr_box_exception"})
        return None
...
def run_ocr_easyocr_improved(crop_img, enable_post_processing: bool = True, char_mappings: dict = None):
    ...
    Returns:
        tuple: (processed_text, replacements_list, confidence_score)
    """
    try:
        # EasyOCR Reader initialisieren (Singleton-Pattern)
        if not hasattr(run_ocr_easyocr_improved, 'reader'):
            run_ocr_easyocr_improved.reader = easyocr.Reader(['en'], gpu=False)
            log_ocr.info("Reader für improved init", extra={"lang": ["en"], "gpu": False, "event": "ocr_reader_improved"})
        ...
        if replacements:
            log_ocr.info("Post-Processing", extra={"replacements": replacements, "event": "ocr_post_processing"})
        
        best_conf = max(confidences) if confidences else 0.0
        return processed_text, replacements, float(best_conf)
        
    except Exception as e:
        log_ocr.exception("EasyOCR Improved Error", extra={"event": "ocr_improved_exception"})
...
def old_ocr_method(image_path, valid_kurzel, alternative_kurzel=None, enable_char_replacements=True, enable_number_normalization=True, fuzzy_cutoff=0.7):
    ...
    try:
        ...
        # Character Replacements nur wenn aktiviert
        ...
        # Zahlen-Normalisierung nur wenn aktiviert
...
def process_single_image_for_batch(args: dict) -> dict:
    ...
    Verwendet die alte OCR-Methode ohne automatische Box-Suche.

    Args dict keys:
        - path: str
        - dp: dict (DetectParams as dict)
        - valid_kurzel: list[str]
        - alternative_kurzel: dict
        - enable_char_replacements: bool
        - enable_number_normalization: bool
        - fuzzy_cutoff: float
    ...
    try:
        ...
        # Alte OCR-Methode verwenden (ohne automatische Box-Suche)
        ocr_result = old_ocr_method(
            path, 
            valid_kurzel,
        ...
        result = {
            ...
            'search_rect': None,  # Keine Suchbereich
