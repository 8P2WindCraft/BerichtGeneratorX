...
class ExcelGrunddatenDialog:
    ...
    def load_excel_data(self, file_path):
        ...
        try:
            ...
            print(f"  Windpark: {windpark_col}")
            print(f"  Land: {land_col}")
            print(f"  Seriennummer: {sn_col}")
            print(f"  ID: {id_col}")
            print(f"  Hersteller: {hersteller_col}")
            ...
            for i, filename in enumerate(image_files):
                ...
                try:
                    ...
                    with Image.open(file_path) as img:
                        ...
                        new_exif = dict(exif_data)  # Konvertiere zu dict statt copy()
                        new_exif[0x9286] = new_user_comment  # UserComment Tag
                        
                        # Bild mit neuen Exif-Daten speichern
...
class OCRReviewApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Button(left_buttons, text="Excel Grunddaten laden", command=self.show_excel_dialog).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(left_buttons, text="Analysieren", command=self.auto_analyze).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(left_buttons, text="Aktualisieren", command=self.refresh_images).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(left_buttons, text="EXIF anzeigen", command=self.show_exif_info).pack(side=tk.LEFT, padx=(0, 5))
        ...
        self.right_buttons.pack(side=tk.RIGHT, fill=tk.X)
        ...
        ttk.Button(self.right_buttons, text="Kürzel-Tabelle", command=self.open_kurzel_table_manager).pack(side=tk.RIGHT, padx=(5, 0))
        ...
        prev_button = tk.Button(nav_center_frame, text="◀ Vorher", command=self.prev_image, 
                                bg="#2196F3", fg="white", font=("TkDefaultFont", 12, "bold"), width=15)
        ...
        zoom_button = tk.Button(nav_center_frame, text="Zoom & Markieren", command=self.open_zoom_window, 
                               bg="#FF9800", fg="white", font=("TkDefaultFont", 12, "bold"), width=18)
        ...
        next_button = tk.Button(nav_center_frame, text="Nächste ▶", command=self.next_image, 
                                bg="#9C27B0", fg="white", font=("TkDefaultFont", 12, "bold"), width=15)
        ...
        ok_button = tk.Button(nav_center_frame, text="✓ In Ordnung", command=self.mark_as_ok_and_next, 
                              bg="#4CAF50", fg="white", font=("TkDefaultFont", 12, "bold"), width=15)
        ...
        use_frame.grid(row=0, column=0, sticky="ew", pady=(0, 5))
        ...
        damage_frame.grid(row=1, column=0, sticky="ew", pady=(0, 5))
        self.damage_vars = {}
        ...
        image_type_frame.grid(row=2, column=0, sticky="ew", pady=(0, 5))
        ...
        quality_frame = ttk.LabelFrame(center_column, text="Schadensbewertung", padding=5)
        quality_frame.grid(row=3, column=0, sticky="ew")
        ...
        right_column.grid_rowconfigure(0, weight=1)  # Erste Zeile expandiert vertikal
        
        counts_frame = ttk.LabelFrame(right_column, text="Anzahl", padding=5)
        counts_frame.grid(row=0, column=0, sticky="nsew")
    ...
    def migrate_existing_kurzel_to_table(self):
        ...
        try:
            ...
            config_kurzel = self.config.get_setting('valid_kurzel', [])
            ...
            for kurzel_code in all_kurzel:
                ...
                if not self.config.kurzel_table_manager.get_kurzel(kurzel_code):
                    # Erstelle Standard-Struktur für das Kürzel
                    kurzel_data = self.config.kurzel_table_manager.get_default_kurzel_structure()
                    ...
                    if self.config.kurzel_table_manager.add_kurzel(kurzel_data):
                        migrated_count += 1
    ...
    def open_kurzel_table_manager(self):
        ...
        columns = ('kurzel_code', 'name_de', 'name_en', 'category', 'image_type', 'active')
        ...
        tree.heading('image_type', text='Bildtyp')
        ...
        tree.column('image_type', width=150)
        ...
        kurzel_data = self.config.kurzel_table_manager.get_all_kurzel()
        if not kurzel_data:
    ...
    def filter_kurzel_table(self, search_term, win):
        ...
        if not search_term:
            ...
            # Suche Kürzel
            kurzel_data = self.config.kurzel_table_manager.search_kurzel(search_term)
    ...
    def delete_kurzel_from_table(self, win):
        ...
        if messagebox.askyesno("Bestätigung", f"Möchten Sie das Kürzel '{kurzel_code}' wirklich löschen?"):
            if self.config.kurzel_table_manager.delete_kurzel(kurzel_code):
                self.load_kurzel_table_data(win)
    ...
    def kurzel_edit_dialog(self, parent, kurzel_code=None):
        ...
        if kurzel_code:
            data = self.config.kurzel_table_manager.get_kurzel(kurzel_code)
            if not data:
                messagebox.showerror("Fehler", "Kürzel nicht gefunden.")
                dialog.destroy()
                return
        else:
            data = self.config.kurzel_table_manager.get_default_kurzel_structure()
        ...
        def save_kurzel():
            ...
            if kurzel_code:
                ...
                if self.config.kurzel_table_manager.update_kurzel(kurzel_code, kurzel_data):
    ...
    def export_kurzel_table(self):
        ...
        if filename:
            exported_file = self.config.kurzel_table_manager.export_to_csv(filename)
            if exported_file:
    ...
    def import_kurzel_table(self, parent):
        ...
        if filename:
            if messagebox.askyesno("Bestätigung", "Möchten Sie die bestehenden Kürzel durch die importierten ersetzen?"):
                imported_count = self.config.kurzel_table_manager.import_from_csv(filename)
                if imported_count > 0:
    ...
    def open_config_editor(self):
        ...
        # Schadensbewertungs-Kategorien
        ttk.Label(kategorien_frame, text="Schadensbewertungs-Kategorien (eine pro Zeile):").pack(anchor='w', padx=5, pady=(5,0))
    ...
    def open_folder(self):
        ...
        # Aktualisiere Bewertungsfortschritt und Code-Listbox
        self.update_evaluation_progress()
        self.update_code_listbox()
    ...
    def mark_as_ok_and_next(self):
        ...
        try:
            ...
            first_damage_category = self.config.get_language_specific_list('damage_categories')[0]
